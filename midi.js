! function (e) {
  ...
  // Retrives the patch file!!! Changed to return a promise.
  function o(e, n, t) {
  ...
  // Seems to be some "make request" function that takes in a URL. called at the very bottom
  function a(e) {
  ...
  function c(e) { // changed, init ternary ops, not if/else
    S || (window.AudioContext = window.AudioContext || window.webkitAudioContext, S = new AudioContext);
    if (S.resume) {
      S.resume();
      d(e);
    } else {
  ...
  // Changed to return promises of missing instruments, now retrieving.
  // For some reason, n() = f() ???. not sure why
  function f(e, n, t) {
    ...
    if (W = Module.ccall("mid_song_load", "number", ["number", "number"], [a, u]), rval = Module.ccall("mid_istream_close", "number", ["number"], [a]), F = Module.ccall("mid_song_get_num_missing_instruments", "number", ["number"], [W]), 0 < F) {
      // Calls the retrieval on missing instruments!
      for (var l = 0; l < F; l++) {
        var c = Module.ccall("mid_song_get_missing_instrument", "string", ["number", "number"], [W, l]);
        promises.push(o(e, q + "pat/", c)) // Probably the full link, e.g. http://asdf.com/pat/arachno-127
      }
      // console.log(promises);
    } else {
      Module.ccall("mid_song_start", "void", ["number"], [W]), V = S.createScriptProcessor(R, 0, 1), O = Module._malloc(2 * R), V.onaudioprocess = r, V.connect(S.destination), P = setInterval(i, J), MIDIjs.message_callback && MIDIjs.message_callback("Playing: " + e), A("Playing: " + e + " ...")
    }
    // Basically trying to return all the promises...
    // console.log(promises); // Works here... but not outside.
    return promises;
  }
  ...
  // Returns the root website directory (ex: http://0.0.0.0:8000/)
  function x() {
